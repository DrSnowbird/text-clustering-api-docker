...
import pandas as pd
import string
from flask import Flask, jsonify, request, escape, Response, make_response
    ...
    if words:
        if words.strip():
            for word in words.split(' '):
                if (word.strip().lower() == 'email') or (word.strip().lower() == 'phn') or (
                        word.strip().lower() == 'phone') or (len(word.strip()) <= 1):
                    continue
...
def index():
    ...
    Call this api passing the following parameters -
        ...
        Number of Clusters
    Sample URL: http://0.0.0.0:8180/unguided_cluster?dataset=http://192.168.0.127:18080/jetty_base/webapps/data/text/abcnews-mini-text.csv&no_of_clusters=30&col=headline_text
    ---
    tags:
      - Clustering API
    parameters:
      - name: dataset
        ...
        type: file
        required: false
        description: The fully qualified local full file path of the dataset without the extension.
      - name: dataset
        in: query
        type: string
        required: false
        description: The fully qualified URL of the dataset without the extension.
      - name: col
        in: query
        type: string
        required: true
    ...
    if 'dataset' in request.args:
        ...
        # url = "http://0.0.0.0:18080/jetty_base/webapps/data/text/phrases.csv"
    ...
    if dataset:
        ...
        print("*** ERROR: Request does not have 'dataset' either Request's Keyworkd or Argument! Abort!")
        abort(400)
    ...
    kmeans = KMeans(n_clusters=no_of_clusters)  # <deprecated>: ,n_jobs=-1)
...
def index_guided():
    ...
    parameters:
      ...
      - name: phrases
        ...
        type: string
        required: false
        description: The fully qualified URL of the phrases file (single column without header line) without the extension.
    ...
    # ------------------------------
    # Phrases (keywords) URL or File
    # ------------------------------
    phrases = ''
    if 'phrases' in request.args:
        phrases = request.args.get('phrases')
        # url = "http://0.0.0.0:18080/jetty_base/webapps/data/text/phrases.csv"
        print("phrases=", phrases)

    if 'phrases' in request.files:
        phrases = request.files['phrases']

    if phrases:
        print("dataset=", dataset)
    else:
        print("*** ERROR: Request does not have 'phrases' either Request's Keyworkd or Argument! Abort!")
        abort(400)
    data_keywords = pd.read_csv(phrases, header=None)
